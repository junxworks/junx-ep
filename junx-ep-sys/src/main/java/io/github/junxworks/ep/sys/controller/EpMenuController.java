...
public class EpMenuController {
	...
	public Result getForMenu(String username) {
		...
		final Map<Long, MenuInfoVo> dirs = userMenuList.stream().filter(m -> {
			if (m.getType() == MenuType.DIRECTORY.getValue()) {
				if (m.getChildren() == null) {
					m.setChildren(Lists.newArrayList());
				}
			}
	...
	public Result getMenuTree() {
		List<MenuInfoVo> userMenuList = menuService.getAllMenuItems();
		final Map<Long, MenuInfoVo> root = userMenuList.stream().filter(m -> {
			return m.getParentId() == 0;
		}).collect(Collectors.toMap(MenuInfoVo::getId, m -> m));
		final Map<Long, MenuInfoVo> dirs = userMenuList.stream().collect(Collectors.toMap(MenuInfoVo::getId, m -> m));
		userMenuList.stream().sorted((m1, m2) -> {
			int s1 = 0, s2 = 0;
			if (m1.getSort() != null)
				s1 = m1.getSort();
