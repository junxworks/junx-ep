 ...
 */
@Mapper
public interface EpMenuMapper extends BaseMapper {
	...
	@Select("select * from ep_s_menu where status=0 and mark=#{mark}")
	public EpSMenu queryMenuItemByMark(@Param("mark") String mark);
	 ...
	 * @return the page
	 */
	@Select("<script>" + "select * from ep_s_menu where 1=1" + "<if test='status!=null '> " + "and status = #{status} " + "</if>" + "<if test='name!=null and name.length>0 '> " + "and name like CONCAT('%',#{name},'%') " + "</if>" + "<if test='parentId!=null and parentId.length>0 '> " + "and parent_id = #{parentId} " + "</if>" + "<if test='type!=null and type.length>0 '> " + "and type = #{type} "
			+ "</if>" + " order by sort asc" + "</script>")
	 ...
	 * @return the list
	 */
	@Select("select DISTINCT m.* from ep_s_menu m,ep_s_role_menu rm,ep_s_role r where rm.role_id=#{roleId} and rm.menu_id = m.id and rm.role_id=r.id and r.status=0 and m.status=0")
	 ...
	 * @return the int
	 */
	@Select({ "<script>", "select count(1) from ep_s_menu where status=0 and parent_id=#{id}", "</script>" })
	 ...
	 */
	@Select("<script> " + "SELECT m.* FROM " + "(SELECT DISTINCT rm.menu_id FROM ep_s_role_menu rm,ep_s_user_role ur,ep_s_role r WHERE r.status=0 and r.id=rm.role_id and rm.role_id = ur.role_id and ur.user_id =#{userId}) menus" + ",ep_s_menu m WHERE menus.menu_id = m.id and m.type!=1 AND  m.`status`=0" + "</script>")
