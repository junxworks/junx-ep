 ...
 */
@Mapper
public interface EpOrgMapper extends BaseMapper {
	 ...
	 */
	@Select("<script>" 
			+"select * from ep_s_org where status=0 "
			+ "<if test='rootNo!=null and rootNo.length>0'>" 
			+" 	 and org_no like CONCAT('%',#{rootNo},'%') " 
			+"</if>"
		+"</script>"
		)
